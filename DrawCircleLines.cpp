...
LRESULT WndProc(HWND hwnd, UINT m, WPARAM wp, LPARAM lp)
{
    ...
    case WM_LBUTTONUP:
    {
        ...
        // Draw Line in 1 and 3 part
        MoveToEx(hdc, Xc - (1/sqrt(2) * R), Yc - (1 / sqrt(2) * R), NULL);
        LineTo(hdc, Xc + (1 / sqrt(2) * R), Yc + (1 / sqrt(2) * R));

        // Draw horizontal line
        MoveToEx(hdc, Xc - R, Yc, NULL);
        LineTo(hdc, Xc + R, Yc);

        // Draw Line in 2 and 4 part
        MoveToEx(hdc, Xc + (1 / sqrt(2) * R), Yc - (1 / sqrt(2) * R), NULL);
        LineTo(hdc, Xc - (1 / sqrt(2) * R), Yc + (1 / sqrt(2) * R));
...
int APIENTRY WinMain(HINSTANCE hi, HINSTANCE pi, LPSTR cmd, int nsh)
{
    ...
    HWND hwnd = CreateWindow(L"MyClass", L"Draw My Circle With Lines ^_^!", WS_OVERLAPPEDWINDOW, CW_USEDEFAULT, CW_USEDEFAULT, 1000, 600, NULL, NULL, hi, 0);
    ShowWindow(hwnd, nsh);
    UpdateWindow(hwnd);
